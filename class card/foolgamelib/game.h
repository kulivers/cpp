...
void Turn(vector<CPlayer>& _players, vector<CCard>& deck, int FirstTurnPlayer)//в цикле после первого хода
{
	...
	// сделать так, что если колличество карт на столе - четное значение, то кидает карту ѕодкидывающий 1 или 2(только одну), а если нечетное то ƒефендер отбиваетс€
	// цикл пока подкидывальщики могут подкинуть подкидывают )0)) а дефендер отбиваетс€ если может

	// здесь дефендер бьетс€
...
void TurnForTwoPlayers(vector<CPlayer>& _players, vector<CCard>& deck, int AttackPlayer)
{
	...
	CCard lastcard = DropToTableRandCard(_players[AttackPlayer]); //здесь атакер кидает под дефуендера 
	cout << "»грок " << AttackPlayer + 1 << " кидает на стол: " << lastcard.GetNumb() << " " << lastcard.GetSuit() << endl;
	...
	cout << "»грок " << DefendPlayer + 1 << " кидает на стол: ";
	_players[DefendPlayer].BeatOneCard(lastcard);//ќ 
	CCard dropToTable;



	while (_players[AttackPlayer].CanPopUp()) //«десь cоответс€твенно тоже
	{
		//если на столе четное колличество карт то подкидывают одну карту
